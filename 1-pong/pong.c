...
void update_pong(struct Pong* pong, double dt)
{
    if (pong->state == PLAY)
    {
        {   // paddle AI for player 1
            
            float paddle_center = pong->player1.y + PADDLE_HEIGHT * 0.5;
            
            if (pong->ball.vx < 0 && pong->ball.y > paddle_center) {
                pong->player1.vy = PADDLE_SPEED;
                
            }
            else if(pong->ball.vx < 0 && pong->ball.y < paddle_center) {
                pong->player1.vy = -PADDLE_SPEED;
        ...
        {   // paddle AI for player 2
            
            float paddle_center = pong->player2.y + PADDLE_HEIGHT * 0.5;
...
{
    // color palette: https://colorhunt.co/palette/08d9d6252a34ff2e63eaeaea
    
    al_draw_filled_rectangle(
        TABLE_WIDTH / 2 - MID_LINE_WIDTH / 2, 0,
        TABLE_WIDTH / 2 + MID_LINE_WIDTH / 2, TABLE_HEIGHT,
        al_map_rgb(255, 46, 99)
    );
    ...
    al_draw_text(fonts.score_font, al_map_rgb(8, 217, 214), TABLE_WIDTH / 2 - 50, TABLE_HEIGHT / 6, ALLEGRO_ALIGN_CENTER, score);
    sprintf(score, "%d", pong.player2_score);
    al_draw_text(fonts.score_font, al_map_rgb(8, 217, 214), TABLE_WIDTH / 2 + 50, TABLE_HEIGHT / 6, ALLEGRO_ALIGN_CENTER, score);

    if (pong.state == START)
    {
        al_draw_text(fonts.large_font, al_map_rgb(8, 217, 214), TABLE_WIDTH / 2, TABLE_HEIGHT / 2, ALLEGRO_ALIGN_CENTER, "Press enter to start");
    }
    else if (pong.state == SERVE)
    {
        al_draw_text(fonts.large_font, al_map_rgb(8, 217, 214), TABLE_WIDTH / 2, TABLE_HEIGHT / 2, ALLEGRO_ALIGN_CENTER, "Press enter to serve");
    }
    else if (pong.state == DONE)
    {
        char winner_message[14];
        sprintf(winner_message, "Player %d won!", pong.winning_player);
        al_draw_text(fonts.large_font, al_map_rgb(8, 217, 214), TABLE_WIDTH / 2, TABLE_HEIGHT / 3, ALLEGRO_ALIGN_CENTER, winner_message);
        al_draw_text(fonts.large_font, al_map_rgb(8, 217, 214), TABLE_WIDTH / 2, TABLE_HEIGHT / 2, ALLEGRO_ALIGN_CENTER, "Press enter to restart");
    }
}
